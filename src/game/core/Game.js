export class Game {
  ...
  updateUI () {
    ...
    document.getElementById('level').textContent = this.state.level.toString().padStart(2, '0');
  ...
  static createGameCharacters () {
    const { Swordsman, Bowerman, Daemon } = require('../entities/index.js');

    const player = new Swordsman('Player');
    const enemy1 = new Bowerman('EnemyArcher');
    const enemy2 = new Daemon('EnemyDaemon');

    return { player, enemies: [enemy1, enemy2] };
  }
